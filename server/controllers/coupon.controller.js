...
export const validateCoupon = async (req, res) => {
    try {
      const { code, totalCost } = req.body;  // Expecting totalCost in the request body
      ...
      if (coupon.percentDiscount) {
        voucherDiscount += (coupon.percentDiscount / 100) * totalCost;
      }
      ...
      return res.status(200).json({
        discountedCost,
        voucherDiscount,
        message: `Voucher applied: ${voucherDiscount} VNĐ off`,
      });
    } catch (error) {
